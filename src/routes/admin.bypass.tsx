...
import { Plus, Trash2, Copy, Loader2, RefreshCcw, X, ChevronDown, Info, RefreshCw, Check, Calendar, Clock, Shield, Hash, Zap, Activity, Ban, Trash, Search, Filter, Bell, BellOff } from "lucide-react";
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Manage Free Fire UID Whitelisting, Blacklisting, and Proxy Logs."
        action={
          <div className="flex items-center gap-3 flex-wrap">
            {settings && (
              <button
                onClick={() => {
                  const isActive = settings.discord_log_bypass !== "false";
                  const newSettings = { ...settings, discord_log_bypass: isActive ? "false" : "true" };
                          ...
                          <td className="px-6">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <div className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest mt-0.5">By: {item.added_by || "Admin"}</div>
                              </div>
                            </div>
                          </td>
                          <td className="px-6">
                  ...
                  filteredBlacklist.map((item: any, idx: number) => (
                    <div key={item.uid} className="p-5 space-y-4 hover:bg-white/[0.01] transition-colors relative">
                      <div className="flex justify-between items-start">
                        ...
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border border-red-500/20 bg-red-500/10 text-red-400">
                          BANNED
                        </span>
                      </div>
